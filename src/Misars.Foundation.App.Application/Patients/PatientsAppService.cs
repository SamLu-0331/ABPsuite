...
namespace Misars.Foundation.App.Patients
{
    ...
    public abstract class PatientsAppServiceBase : AppAppService
    {
        ...
        {
            var totalCount = await _patientRepository.GetCountAsync(input.FilterText, input.name, input.birthdayMin, input.birthdayMax, input.phone);
            var items = await _patientRepository.GetListAsync(input.FilterText, input.name, input.birthdayMin, input.birthdayMax, input.phone, input.Sorting, input.MaxResultCount, input.SkipCount);
        ...
        public virtual async Task<PatientDto> CreateAsync(PatientCreateDto input)
        {
            ...
            input.birthday, input.name, input.phone
            );
        ...
        public virtual async Task<PatientDto> UpdateAsync(Guid id, PatientUpdateDto input)
        {
            ...
            id,
            input.birthday, input.name, input.phone, input.ConcurrencyStamp
            );
        ...
        public virtual async Task<IRemoteStreamContent> GetListAsExcelFileAsync(PatientExcelDownloadDto input)
        {
            ...
            var items = await _patientRepository.GetListAsync(input.FilterText, input.name, input.birthdayMin, input.birthdayMax, input.phone);
        ...
        {
            await _patientRepository.DeleteAllAsync(input.FilterText, input.name, input.birthdayMin, input.birthdayMax, input.phone);
        }
