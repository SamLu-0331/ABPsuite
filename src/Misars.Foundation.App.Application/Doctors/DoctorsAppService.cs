...
namespace Misars.Foundation.App.Doctors
{
    ...
    public abstract class DoctorsAppServiceBase : AppAppService
    {
        ...
        {
            var totalCount = await _doctorRepository.GetCountAsync(input.FilterText, input.name, input.email, input.notes);
            var items = await _doctorRepository.GetListAsync(input.FilterText, input.name, input.email, input.notes, input.Sorting, input.MaxResultCount, input.SkipCount);
        ...
        public virtual async Task<DoctorDto> CreateAsync(DoctorCreateDto input)
        {
            ...
            input.name, input.email, input.notes
            );
        ...
        public virtual async Task<DoctorDto> UpdateAsync(Guid id, DoctorUpdateDto input)
        {
            ...
            id,
            input.name, input.email, input.notes, input.ConcurrencyStamp
            );
        ...
        public virtual async Task<IRemoteStreamContent> GetListAsExcelFileAsync(DoctorExcelDownloadDto input)
        {
            ...
            var items = await _doctorRepository.GetListAsync(input.FilterText, input.name, input.email, input.notes);
        ...
        {
            await _doctorRepository.DeleteAllAsync(input.FilterText, input.name, input.email, input.notes);
        }
