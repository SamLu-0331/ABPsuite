...
namespace Misars.Foundation.App.Blazor.Client.Pages
{
    public partial class Doctors
    {
        ...
        private async Task DownloadAsExcelAsync()
        {
            ...
            NavigationManager.NavigateTo($"{remoteService?.BaseUrl.EnsureEndsWith('/') ?? string.Empty}api/app/doctors/as-excel-file?DownloadToken={token}&FilterText={HttpUtility.UrlEncode(Filter.FilterText)}{culture}&name={HttpUtility.UrlEncode(Filter.name)}&email={HttpUtility.UrlEncode(Filter.email)}&notes={HttpUtility.UrlEncode(Filter.notes)}", forceLoad: true);
        }
