...
namespace Misars.Foundation.App.Blazor.Client.Pages
{
    public partial class Patients
    {
        ...
        private async Task DownloadAsExcelAsync()
        {
            ...
            NavigationManager.NavigateTo($"{remoteService?.BaseUrl.EnsureEndsWith('/') ?? string.Empty}api/app/patients/as-excel-file?DownloadToken={token}&FilterText={HttpUtility.UrlEncode(Filter.FilterText)}{culture}&name={HttpUtility.UrlEncode(Filter.name)}&birthdayMin={Filter.birthdayMin?.ToString("O")}&birthdayMax={Filter.birthdayMax?.ToString("O")}&phone={HttpUtility.UrlEncode(Filter.phone)}", forceLoad: true);
        }
        ...
        protected virtual async Task OnbirthdayMinChangedAsync(DateTime? birthdayMin)
        {
            Filter.birthdayMin = birthdayMin.HasValue ? birthdayMin.Value.Date : birthdayMin;
            await SearchAsync();
        }
        protected virtual async Task OnbirthdayMaxChangedAsync(DateTime? birthdayMax)
        {
            Filter.birthdayMax = birthdayMax.HasValue ? birthdayMax.Value.Date.AddDays(1).AddSeconds(-1) : birthdayMax;
            await SearchAsync();
        }
